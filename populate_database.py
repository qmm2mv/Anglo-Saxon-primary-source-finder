...
async def load_single_file(file_path, executor):
    ...
    FIXED: Now handles encoding for text files properly.
    """
    ...
    try:
        ...
        # FIXED: Try multiple encodings for text files
        if loader_class_name == "TextLoader":
            ...
            for encoding in encodings_to_try:
                try:
                    ...
                    logging.info(f"Loaded {filename} with {encoding} encoding")
                    return documents
                except Exception as e:
                    last_error = e
                    continue

            # If all encodings failed, log the error
            if documents is None:
                logging.error(f"Error loading {file_path} with all encodings. Last error: {str(last_error)}")
                return []
        else:
            # Non-text files use normal loading
